...
public class UserDaoImpl implements UserDao {
		...
		try {
			...
			if (resultSet.next()) {
				user = new User(resultSet.getInt(1), resultSet.getString(2), resultSet.getString(3),
						...
						UserType.valueOf(resultSet.getString(6))/* ,resultSet.getTimestamp(7).toLocalDateTime() */);
		...
		try {
			...
			if (userByEmail != null) {
				...
				if (userByEmail.getPassword() == null) {
					...
					throw new AuthenticationException("Unable to Login User, check credentials");
				}
	...
	public void logoutUser(int userId) throws DataAccessException, AuthenticationException {
		...
		try {
			...
			throw new AuthenticationException("User Not found with id");
		} finally {
			try {
				connection.close();
			} catch (SQLException e) {
	...
	public void registerUser(String email, String password)
			throws UserAlreadyRegisteredException, DataAccessException, UserNotFoundException {
		try {
			User user = getUserByEmail(email);
			if (user.getPassword() != null) {
				throw new UserAlreadyRegisteredException(
						"User with email " + email + " already Registired , please proceed to login");
			} else {
